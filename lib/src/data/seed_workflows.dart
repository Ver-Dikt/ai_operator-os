...
WorkflowStep _step(String id, String title, String instruction, {String? agentId, List<String> tools = const [], String prompt = '', bool manual = true, bool auto = false}) {
  return WorkflowStep(
    id: id,
    title: title,
    ...
    agentId: agentId,
    toolIds: tools,
    promptTemplate: prompt.isEmpty ? 'Task: {{task}}\nConstraints: {{constraints}}\nOutput: $title' : prompt,
...
final seedWorkflows = <WorkflowTemplate>[
  WorkflowTemplate(
    ...
    description: 'Idea to hook, script, scenes, video prompts, tool selection, edit plan and captions.',
    category: 'Video',
    ...
    steps: [
      _step('idea', 'Idea', 'Condense the idea into one emotional promise.', agentId: 'content-factory-agent'),
      _step('hook', 'Hook', 'Write three first-second hooks and pick the strongest constraint.'),
      _step('script', 'Script', 'Write a compact script with a clear turn.'),
      _step('scenes', 'Scenes', 'Split the script into visual beats.', agentId: 'director-agent'),
      _step('video-prompts', 'Video Prompts', 'Write prompt-ready shots with blocking and camera reason.', tools: ['kling', 'veo']),
      _step('tool-selection', 'Tool Selection', 'Choose free, fast and quality tool paths.', agentId: 'tool-router-agent'),
      _step('generation', 'Generation Checklist', 'Track generated shots and variations.'),
      _step('edit', 'Edit Plan', 'Define rhythm, captions and final gesture.'),
      _step('publish', 'Publish Captions', 'Write captions and hashtags for release.'),
    ],
  ),
  WorkflowTemplate(
    id: 'music-release-promo-pack',
    title: 'Music Release Promo Pack',
    description: 'Turns a track mood into teaser ideas, cover prompts, lyric clips and posting calendar.',
    category: 'Music',
    ...
    steps: [
      _step('song-mood', 'Song Mood', 'Describe the emotional world of the track.', agentId: 'music-promo-agent'),
      _step('identity', 'Visual Identity', 'Define colors, objects, pacing and texture.'),
      ...
      _step('video', 'Short Video Prompts', 'Create video prompts for promo clips.'),
  ...
  WorkflowTemplate(
    ...
    description: 'Convert a task and constraints into a free/paid/local tool stack.',
    category: 'Research',
    difficulty: WorkflowDifficulty.easy,
    ...
    steps: [
      ...
      _step('constraints', 'Constraints', 'Add budget, platform and time limits.'),
      _step('split', 'Free/Paid Split', 'Separate zero-cost, paid and local options.', agentId: 'free-stack-agent'),
      _step('best-tools', 'Best Tools', 'Pick tools by fit and reliability.', agentId: 'tool-router-agent'),
      _step('workflow', 'Workflow Recommendation', 'Turn the result into a sequence.'),
    ],
  ),
  WorkflowTemplate(
    id: 'cinematic-scene-builder',
    title: 'Cinematic Scene Builder',
    description: 'Builds a scene with dramatic beat, blocking, camera reason and final gesture.',
    category: 'Video',
    ...
    steps: [
      ...
      _step('blocking', 'Blocking', 'Place bodies before camera motion.', agentId: 'director-agent'),
      _step('camera', 'Camera Reason', 'Move the camera only for a dramatic reason.'),
      _step('stable', 'Stable Shot', 'Use stability as an expressive choice.'),
      _step('gesture', 'Final Gesture', 'Define the gesture that reframes the scene.'),
  ...
  WorkflowTemplate(
    ...
    description: 'Prepare a local stack around Ollama, model selection and local knowledge.',
    category: 'Local',
    difficulty: WorkflowDifficulty.medium,
